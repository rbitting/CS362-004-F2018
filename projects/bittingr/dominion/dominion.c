...
void adventurerEffect(int drawntreasure, struct gameState *state, int temphand[], int currentPlayer) {
	int cardDrawn;
	int z = 0;
...
void councilRoomEffect(int handPos, int currentPlayer, struct gameState *state) {
	//+4 Cards
	int i;
    for (i = 0; i < 4; i++)
	{
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      adventurerEffect(drawntreasure, state, temphand, currentPlayer);
      return 0;
